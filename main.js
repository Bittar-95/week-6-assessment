...
function decreseBy(arr,decrease) {
	var result = arr.map(x=>x-decrease);
	return result; 
...
function nameLongerThan(arr,length) {

	var result = arr.filter(x=>x.name.length>length);
	return result;
...
function avgLength(arr,name) {
	...
	var result = arr.reduce(function (x,y,z) {
		x=x+y[name].length;
